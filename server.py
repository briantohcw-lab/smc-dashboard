...
<header>
  ...
  <div class="header-right">
    ...
    <div id="creditMeter" title="Twelve Data API credits used today"
         style="font-size:10px;letter-spacing:0.08em;padding:3px 8px;border:1px solid var(--border2);border-radius:3px;color:var(--text2);white-space:nowrap">
      ⛁ --/-- credits
    </div>
...
<div class="controls">
  ...
  <div class="filter-group">
    ...
    <button class="filter-btn"        id="cf-4" onclick="setConf(4)">4+</button>
  </div>
...
<div class="legend">
  ...
  <div class="legend-item"><div class="legend-dot" style="background:var(--dim)"></div>Dots = confluence score /5</div>
</div>

<!-- ── MAIN GRID ───────────────────────────────────── -->
<!-- ── SCAN PROGRESS ── -->
<div id="scanProgressWrap" style="display:none;padding:6px 24px;border-bottom:1px solid var(--border);background:rgba(59,130,246,0.05)">
  <div style="display:flex;align-items:center;gap:10px">
    <span id="scanProgressText" style="font-size:9px;color:var(--accent);letter-spacing:0.12em;white-space:nowrap">SCANNING...</span>
    <div style="flex:1;height:3px;background:var(--border);border-radius:2px;overflow:hidden">
      <div id="scanProgressBar" style="height:100%;width:0%;background:linear-gradient(90deg,var(--accent),var(--bull));transition:width 0.4s ease"></div>
    </div>
    <span id="scanProgressPct" style="font-size:9px;color:var(--text2);white-space:nowrap">0%</span>
  </div>
</div>
...
<div style="padding:0 24px 24px">
  <div class="section-label" style="margin-bottom:10px">
    ◎ APPROACHING OB — WATCHLIST <span class="count-pill" id="watchCount">0</span>
  </div>
  <div id="watchTable" style="border:1px solid var(--border2);border-radius:3px;overflow:hidden">
    <div class="empty-col" style="border:none">NO PAIRS APPROACHING<div class="pulse-text">SCANNING...</div></div>
  </div>
</div>
...
<div style="padding:0 24px 28px">
  <div class="section-label" style="margin-bottom:10px">
    ⏱ HISTORY — 2+ CONFLUENCE (LAST 10) <span class="count-pill" id="histCount">0</span>
  </div>
  <div id="histTable" style="border:1px solid var(--border2);border-radius:3px;overflow:hidden">
    <div class="empty-col" style="border:none">NO HISTORY YET<div class="pulse-text">BUILDS UP AS THE ENGINE SCANS...</div></div>
  </div>
</div>
...
// ── History (past 2+ confluence signals) ────────────
...
function renderHistory(h) {
  ...
  if (!h.length) {
    el.innerHTML = '<div class="empty-col" style="border:none">NO HISTORY YET<div class="pulse-text">BUILDS UP AS THE ENGINE SCANS...</div></div>';
    return;
  }
  const rows = h.map(x => {
    const dirColor = x.bias==='bull' ? 'var(--bull)' : 'var(--bear)';
    const arrow = x.bias==='bull' ? '▲' : '▼';
    const t = x.time ? x.time.slice(5,16).replace('T',' ') : '—';
    const confColor = x.confluence>=3 ? 'var(--warn)' : 'var(--text)';
    return `<div style="display:flex;align-items:center;gap:8px;padding:8px 12px;border-bottom:1px solid var(--border);font-size:11px">
      <span style="color:var(--text2);font-size:9px;min-width:74px">${t}</span>
      <span style="font-weight:600;color:#fff;min-width:60px">${x.pair}</span>
      <span style="color:${dirColor};font-size:9px;letter-spacing:0.08em">${arrow}${x.bias==='bull'?'L':'S'}</span>
      <span style="color:var(--text2);font-size:9px">${x.obType}</span>
      <span style="margin-left:auto;color:${confColor};font-weight:600">${x.confluence}/5</span>
    </div>`;
  }).join('');
  el.innerHTML = rows;
}

// ── Watchlist (pairs approaching an OB) ─────────────
...
function pipLabel(pair, pips) {
  return pips != null ? pips.toFixed(1) + ' pips' : '—';
}

function renderWatch(w) {
  document.getElementById('watchCount').textContent = w.length;
  const el = document.getElementById('watchTable');
  if (!w.length) {
    el.innerHTML = '<div class="empty-col" style="border:none">NO PAIRS APPROACHING<div class="pulse-text">SCANNING...</div></div>';
    return;
  }
  // closeness coloring: <20 pips hot, <50 warm
  const rows = w.map(x => {
    const dirColor = x.bias==='bull' ? 'var(--bull)' : 'var(--bear)';
    const dist = x.distancePips;
    let distColor = 'var(--text2)';
    if (dist < 20) distColor = 'var(--warn)';
    if (dist < 8)  distColor = x.bias==='bull' ? 'var(--bull)' : 'var(--bear)';
    const arrow = x.bias==='bull' ? '▲' : '▼';
    return `<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;border-bottom:1px solid var(--border);font-size:11px">
      <span style="font-weight:600;color:#fff;min-width:64px">${x.pair}</span>
      <span style="color:${dirColor};font-size:9px;letter-spacing:0.1em">${arrow} ${x.bias==='bull'?'LONG':'SHORT'}</span>
      <span style="color:var(--text2);font-size:10px">${x.obType}</span>
      <span style="margin-left:auto;color:var(--text2);font-size:10px">${fmt(x.pair,x.price)}</span>
      <span style="color:${distColor};font-weight:600;min-width:70px;text-align:right">${pipLabel(x.pair,dist)}</span>
    </div>`;
  }).join('');
  el.innerHTML = rows;
}
...
async function doScanNow() {
  toast('⚡ Scan started — watch the progress bar');
  try { await fetch(API + '/scan-now'); } catch(e) {}
  setTimeout(pollStatus, 800);  // start tracking progress quickly
}
...
function conf(s) {
  // Use server-computed confluence if present (out of 5)
  if (typeof s.confluence === 'number') return s.confluence;
  let sc = 1;
  if (s.m15struct) sc++;
  if (s.fvg)  sc++;
  if (s.eqhl) sc++;
...
function emptyCol(dir) {
  return `<div class="empty-col">
    NO ${dir} SIGNALS
    <div class="pulse-text">SCANNING FOR OB CONFLUENCE...</div>
  </div>`;
}
...
function card(s) {
  ...
  return `<div class="card ${s.bias} ${fr?'fresh':''}"
    ...
    ${s.factors && s.factors.length ? `<div style="padding:0 12px 8px;display:flex;gap:4px;flex-wrap:wrap">` + s.factors.map(f=>`<span style="font-size:8px;padding:1px 6px;border-radius:1px;background:rgba(59,130,246,0.12);color:var(--accent);border:1px solid rgba(59,130,246,0.25)">${f}</span>`).join('') + `</div>` : ''}
    <div class="card-grid">
      <div>
    ...
    <div class="card-foot">
      <span style="color:${c>=3?'var(--warn)':'var(--dim)'}">CONF ${c}/5</span>
...
function showTip(e, enc) {
  ...
  document.getElementById('tip').innerHTML = `
    ...
    <div class="tr"><span class="tl">Liquidity Sweep</span>
      <span class="tv" style="color:${s.sweep?'var(--warn)':'var(--dim)'}">${s.sweep?'YES':'NO'}</span></div>
    <div class="tdiv"></div>
    <div class="tr"><span class="tl">Confluence</span>
      <span class="tv" style="color:${c>=3?'var(--warn)':'var(--text)'}">${c}/5</span></div>
...
function setConf(n) {
  cfF = n;
  [0,2,3,4].forEach(x => document.getElementById('cf-'+x).className='filter-btn');
...
fetchWatchlist();
fetchHistory();
setInterval(()=>{ doFetch(); fetchWatchlist(); fetchHistory(); }, 5000);
pollStatus();
render();
...
async function pollStatus() {
  try {
    ...
    // ── credit meter (in scope of s) ──
    const meter = document.getElementById('creditMeter');
    if (meter && s.credits_used != null) {
      const limit = s.credit_limit || 800;
      const pctUsed = s.credits_used / limit;
      let color = 'var(--text2)';
      if (pctUsed >= 0.75) color = 'var(--warn)';
      if (pctUsed >= 0.90) color = 'var(--bear)';
      meter.style.color = color;
      meter.style.borderColor = pctUsed >= 0.75 ? color : 'var(--border2)';
      meter.textContent = `⛁ ${s.credits_used}/${limit} credits`;
    }

    const wrap = document.getElementById('scanProgressWrap');
    const bar  = document.getElementById('scanProgressBar');
    const pct  = document.getElementById('scanProgressPct');
    const ptxt = document.getElementById('scanProgressText');

    if (log.scanning) {
      // show progress bar, poll fast
      const done = log.progress || 0;
      const tot  = log.total_pairs || (s.pairs ? s.pairs.length : 1);
      const percent = Math.round((done/tot)*100);
      wrap.style.display = 'block';
      bar.style.width = percent + '%';
      pct.textContent = percent + '%';
      ptxt.textContent = `SCANNING ${done}/${tot}` + (log.current_pair ? ` — ${log.current_pair}` : '');
      document.getElementById('pollStatus').textContent = `SCANNING ${done}/${tot} PAIRS`;
      schedulePoll(1500);   // poll every 1.5s while scanning
    } else {
      // hide progress bar, poll slow
      ...
      if (log.last_error) txt += ` | WARN ${log.last_error.slice(0,40)}`;
      document.getElementById('pollStatus').textContent = txt;
      schedulePoll(15000);  // poll every 15s when idle
    }
  } catch(e) { schedulePoll(15000); }
}
